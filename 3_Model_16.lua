...
nClasses = 894 -- number of possible classes that can be predicted.
...
-- a typical modern convolution network (conv+relu+pool)
      ...
      -- stage 3 : filter bank
      model:add(nn.SpatialConvolutionMM(ConMatrix3, filtsize, filtsize)
  ...
  model:add(nn.Reshape(nPixels))
  model:add(nn.SplitTable(2,2))
  model:add(nn.JoinTable(1))
  model:add(nn.Reshape(nPixels,nstates[3]))
